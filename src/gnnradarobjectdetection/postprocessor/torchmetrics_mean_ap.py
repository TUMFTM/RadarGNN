...
from gnnradarobjectdetection.utils.math import point_iou, box_area_rotated
...
def compute_area(input: List[Any], iou_type: str = "bbox") -> Tensor:
    ...
    if iou_type == "bbox":
        ...
        box_matrix_tensor = torch.stack(input)
        if box_matrix_tensor.shape[1] == 4:
            # bounding boxes are aligned and in format xyxy

            box_areas = box_area(box_matrix_tensor)

        elif box_matrix_tensor.shape[1] == 5:
            # bounding boxes are rotated and in format cx, cy, l, w, theta_x

            box_areas = box_area_rotated(box_matrix_tensor)

        else:
            raise Exception("Bounding boxes in wrong format")

        # print("Box matrix input: ")
        # print(box_matrix_tensor)
        # print("Returned box areas: ")
        # print(box_areas)
...
def compute_iou(
    ...
    if iou_type == "bbox":
        if use_point_iou:
            ...
            iou_matrix = point_iou(torch.stack(
                det), torch.stack(gt), points, box_aligned)

            # print("----------- results of new class -------------")
            # print("detections: ")
            # print(torch.stack(det))

            # print("ground truth: ")
            # print(torch.stack(gt))

            # print("iou_matrix: ")
            # print(iou_matrix)

            # box_iou_calc = box_iou(torch.stack(det), torch.stack(gt))
            # print(box_iou_calc)

        else:
            iou_matrix = box_iou(torch.stack(det), torch.stack(gt))
            # print("BOX")
            # print(iou_matrix)
...
class MARMetricResults(BaseMetricResults):
    ...
    __slots__ = ("mar_1", "mar_10", "mar_100",
...
def _input_validator(
    ...
    for k in [iou_attribute, "labels"]:
        if any(k not in p for p in targets):
            raise ValueError(
                f"Expected all dicts in `target` to contain the `{k}` key")

    if any(type(pred[iou_attribute]) is not Tensor for pred in preds):
        raise ValueError(
            f"Expected all {iou_attribute} in `preds` to be of type Tensor")
    ...
    if any(type(target[iou_attribute]) is not Tensor for target in targets):
        raise ValueError(
            f"Expected all {iou_attribute} in `target` to be of type Tensor")
    if any(type(target["labels"]) is not Tensor for target in targets):
        raise ValueError(
            "Expected all labels in `target` to be of type Tensor")
...
class MeanAveragePrecision(Metric):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        max_det_thr, _ = torch.sort(
            IntTensor(max_detection_thresholds or [1, 10, 100]))
        ...
        self.add_state("point_clouds", default=[], dist_reduce_fx=None)

    def update(self, preds: List[Dict[str, Tensor]], target: List[Dict[str, Tensor]], use_point_iou=False, point_clouds: List[np.ndarray] = None, is_aligned: bool = True) -> None:  # type: ignore
        ...
        self.use_point_iou = use_point_iou
        self.is_aligned = is_aligned

        if self.use_point_iou:
            for item in point_clouds:
                self.point_clouds.append(item)
    ...
    def _compute_iou(self, idx: int, class_id: int, max_det: int) -> Tensor:
        ...
        gt_label_mask = (
            self.groundtruth_labels[idx] == class_id).nonzero().squeeze(1)
        det_label_mask = (
            self.detection_labels[idx] == class_id).nonzero().squeeze(1)
        ...
        ious = compute_iou(det, gt, self.iou_type,
                           self.use_point_iou, points, self.is_aligned).to(self.device)
        return ious
    ...
    def __evaluate_image_gt_no_preds(
        ...
        det_ignore = torch.zeros(
            (nb_iou_thrs, nb_det), dtype=torch.bool, device=self.device)
    ...
    def __evaluate_image_preds_no_gt(
        ...
        det_ignore_area = (det_areas < area_range[0]) | (
            det_areas > area_range[1])
    ...
    def _summarize(
        ...
        Args:
            ...
            max_dets:
                Maximum detections.
        """
        area_inds = [i for i, k in enumerate(
            self.bbox_area_ranges.keys()) if k == area_range]
        mdet_inds = [i for i, k in enumerate(
            self.max_detection_thresholds) if k == max_dets]
        if avg_prec:
        ...
        mean_prec = torch.tensor(
            [-1.0]) if len(prec[prec > -1]) == 0 else torch.mean(prec[prec > -1])
        return mean_prec
    ...
    def _calculate(self, class_ids: List) -> Tuple[MAPMetricResults, MARMetricResults]:
        ...
        nb_imgs = len(img_ids)
        precision = -torch.ones((nb_iou_thrs, nb_rec_thrs,
                                nb_classes, nb_bbox_areas, nb_max_det_thrs))
        recall = -torch.ones((nb_iou_thrs, nb_classes,
                             nb_bbox_areas, nb_max_det_thrs))
        scores = -torch.ones((nb_iou_thrs, nb_rec_thrs,
                             nb_classes, nb_bbox_areas, nb_max_det_thrs))
    ...
    def _summarize_results(self, precisions: Tensor, recalls: Tensor) -> Tuple[MAPMetricResults, MARMetricResults]:
        ...
        if 0.75 in self.iou_thresholds:
            map_metrics.map_75 = self._summarize(
                results, True, iou_threshold=0.75, max_dets=last_max_det_thr)
        else:
    ...
    def __calculate_recall_precision_scores(
        ...
        det_scores = torch.cat([e["dtScores"][:max_det]
                               for e in img_eval_cls_bbox])
        ...
        det_matches = torch.cat([e["dtMatches"][:, :max_det]
                                for e in img_eval_cls_bbox], axis=1)[:, inds]
        det_ignore = torch.cat([e["dtIgnore"][:, :max_det]
                               for e in img_eval_cls_bbox], axis=1)[:, inds]
        ...
        fps = torch.logical_and(torch.logical_not(
            det_matches), torch.logical_not(det_ignore))
        ...
        for idx, (tp, fp) in enumerate(zip(tp_sum, fp_sum)):
            ...
            while not torch.all(diff == 0):
                ...
                diff = torch.clamp(
                    torch.cat(((pr[1:] - pr[:-1]), diff_zero), 0), min=0)
                pr += diff

            inds = torch.searchsorted(
                rc, rec_thresholds.to(rc.device), right=False)
    ...
    def compute(self) -> dict:
        ...
        if self.class_metrics:
            ...
            for class_idx, _ in enumerate(classes):
                ...
                mar_max_dets_per_class_list.append(
                    cls_mar[f"mar_{self.max_detection_thresholds[-1]}"])

            map_per_class_values = torch.tensor(
                map_per_class_list, dtype=torch.float)
            mar_max_dets_per_class_values = torch.tensor(
                mar_max_dets_per_class_list, dtype=torch.float)
